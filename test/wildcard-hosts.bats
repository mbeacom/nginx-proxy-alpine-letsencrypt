...
# assert that querying nginx-proxy-alpine-letsencrypt with the given Host header produces a `HTTP 200` response
# $1 Host HTTP header to use when querying nginx-proxy-alpine-letsencrypt
function assert_200 {
	local -r host=$1
...
# assert that querying nginx-proxy-alpine-letsencrypt with the given Host header produces a `HTTP 503` response
# $1 Host HTTP header to use when querying nginx-proxy-alpine-letsencrypt
function assert_503 {
	local -r host=$1
